...
@section('title', 'Saacid - Purchase Payments')
@section('admin')
...
<div class="page-btn">
<!-- <a href="{{route('bankStatement.add')}}" class="btn btn-added"><img src="assets/img/icons/plus.svg" alt="img">Add Purchases Payment Transaction</a> -->
</div>
</div>
...
<select name="payment_method" id="payment_method" class="select">
    ...
    <option value="Darasalam Bank">Daarasalaam Bank</option>
</select>
</div>
</div>
<div class="col-lg-3 col-sm-6 col-12">
...
<table class="table" id="PurchasesPaymentTable">
<thead>
<tr class="bg-primary">
...
<th class="text-white">Pay Method</th>
...
<script>
    ...
    let table = $('#PurchasesPaymentTable').DataTable({
        processing: true,
        serverSide: true,
        lengthMenu: [10, 25, 50],
        ajax: {
            url: "{{ route('purchaseTransactions') }}",
            type: 'GET',
            data: function (d) {
                d.name = $('#name').val();
                d.phone = $('#phone').val();
